...
TEST_CASE("qos rxo: source identity refuses regardless of mode")
{
    ...
    for(auto mode : {rxo_mode::permissive, rxo_mode::strict})
    {
        ...
        const auto r = rxo_check(offered, /*offers_source_identity=*/false, req);
    ...
    REQUIRE(rxo_check(offered, /*offers=*/true,
                      subscriber_qos{.durability_mode = durability::none,
                                     .requires_source_identity = true}).verdict
            == rxo_verdict::compatible);
    REQUIRE(rxo_check(offered, /*offers=*/false,
                      subscriber_qos{.durability_mode = durability::none,
                                     .requires_source_identity = false}).verdict
            == rxo_verdict::compatible);
}
...
TEST_CASE("qos rxo: strict mode refuses a soft mismatch with a reason")
{
    ...
    const subscriber_qos one{.durability_mode = durability::none,
                             ...
                             .rxo = rxo_mode::strict};
    const auto r1 = rxo_check(offered, false, one);
    ...
    const subscriber_qos many{.durability_mode = durability::all,
                              .requested_reliability_reliable = true,
                              .rxo = rxo_mode::strict};
    const auto r2 = rxo_check(offered, false, many);
...
TEST_CASE("qos rxo: permissive mode surfaces the degraded field set")
{
    ...
    const auto r = rxo_check(offered, false, req);
    ...
    const subscriber_qos req_dl{.durability_mode = durability::none,
                                .requested_deadline_ns = 100, .rxo = rxo_mode::permissive};
    const auto r_dl = rxo_check(slow, false, req_dl);
    ...
    const auto ok = rxo_check(offered, false,
                              subscriber_qos{.durability_mode = durability::none,
                                             .rxo = rxo_mode::permissive});
