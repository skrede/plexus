...
TEST_CASE("a durability=none subscriber gets ZERO retained frames on subscribe", "[durability][forwarder]")
{
...
TEST_CASE("a durability=latest subscriber gets EXACTLY ONE retained frame", "[durability][forwarder]")
{
    for(int iter = 0; iter < 50; ++iter)
    {
        inproc_bus<>      bus;
