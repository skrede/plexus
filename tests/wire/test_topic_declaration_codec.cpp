...
{
    topic_declaration td{.topic_hash = 0x0102030405060708ULL, .type_id = 0x1112131415161718ULL, .fqn = "sensor/imu", .type_name = "", .state = type_state::undeclared};
...
{
    const topic_declaration td{.topic_hash = 1, .type_id = 2, .fqn = "sensor/imu", .type_name = std::string(300, 'T'), .state = type_state::declared};
...
{
    const topic_declaration td{.topic_hash = 1, .type_id = 2, .fqn = "sensor/imu", .type_name = "sensor/Imu", .state = type_state::declared};
...
{
    const topic_declaration td{.topic_hash = 1, .type_id = 2, .fqn = "sensor/imu", .type_name = std::string(k_max_type_name + 1, 'T'), .state = type_state::declared};

    REQUIRE(!decode_topic_declaration(encode_topic_declaration(td)).has_value());
}

TEST_CASE("topic_declaration codec: an over-cap fqn decodes to nullopt")
{
    const topic_declaration td{.topic_hash = 1, .type_id = 2, .fqn = std::string(k_max_fqn + 1, 'f'), .type_name = "sensor/Imu", .state = type_state::declared};
