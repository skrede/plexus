...
TEST_CASE("an over-cap region length decodes to nullopt", "[wire][subscribe_qos]")
{
    ...
    detail::write_u16(bytes.data() + base_size,
                      static_cast<uint16_t>(detail::k_max_qos_region + 100));
    auto decoded = decode_subscribe_request(bytes);
    CHECK_FALSE(decoded.has_value());
}

TEST_CASE("the protocol version is at 4 and the region is trivially copyable",
          "[wire][subscribe_qos]")
{
