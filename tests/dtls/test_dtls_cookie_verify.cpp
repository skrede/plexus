...
TEST_CASE("dtls.cookie: the cookie verify callback rejects a forged cookie, looped", "[dtls][cookie]")
{
    ...
    const std::byte            addr[] = {std::byte{127}, std::byte{0}, std::byte{0}, std::byte{1}, std::byte{0x1f}, std::byte{0x90}};
    ...
    for(int i = 0; i < k_iterations; ++i)
    {
        ...
        const std::byte other_addr[] = {std::byte{127}, std::byte{0}, std::byte{0}, std::byte{1}, std::byte{0x23}, std::byte{0x28}};
...
TEST_CASE("dtls.failclosed: the ALPN select gate fails closed on a non-overlapping offer, looped", "[dtls][failclosed]")
{
    ...
    for(int i = 0; i < k_iterations; ++i)
    {
        ...
        REQUIRE(ptls::plexus_alpn_select(nullptr, &out, &outlen, offer_other, sizeof(offer_other), nullptr) == SSL_TLSEXT_ERR_ALERT_FATAL);
        ...
        REQUIRE(ptls::plexus_alpn_select(nullptr, &sel, &sel_len, offer_plexus, sizeof(offer_plexus), nullptr) == SSL_TLSEXT_ERR_OK);
        REQUIRE(sel_len == 8);
