...
result run_stamped(std::size_t pool_depth, std::uint64_t iterations, std::uint64_t warm)
{
    ...
    auto r = run_stamp_cell(pool_depth, iterations, warm,
        ...
        { ++delivered; info_sink += info.reception_timestamp; });
...
{
    std::uint64_t delivered = 0;
    return run_stamp_cell(pool_depth, iterations, warm,
...
int main(int argc, char **argv)
{
    ...
    std::printf("    run_stamped   (3-arg, message_info wanted):  per_publish=%.1fns  allocs/publish=%.4f  delivered=%llu\n",
                st.per_pub_ns, st.allocs_per_pub, static_cast<unsigned long long>(st.delivered));
    std::printf("    run_unstamped (2-arg, reception read elided): per_publish=%.1fns  allocs/publish=%.4f  delivered=%llu\n",
                un.per_pub_ns, un.allocs_per_pub, static_cast<unsigned long long>(un.delivered));
    std::printf("    delta (stamped - unstamped) = %.1fns  (the reclaimed reception-clock-read share)\n\n",
