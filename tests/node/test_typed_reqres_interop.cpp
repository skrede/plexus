...
TEST_CASE("typed reqres: a second local serve on one fqn throws on the typed form", "[node][typed][call]")
{
    ...
    typed_procedure proc{n.b, "rpc", [](const request_t &) -> plexus::expected<response_t, std::error_code> { return response_t{0}; }};

    REQUIRE_THROWS_AS((typed_procedure{n.b, "rpc", [](const request_t &) -> plexus::expected<response_t, std::error_code> { return response_t{1}; }}), std::runtime_error);
}
