          ...
          "[outofbox][envelope8]")
{
    ...
    for(int iter = 0; iter < iterations; ++iter)
    {
        ...
        server.on_accepted([&](std::unique_ptr<pasio::asio_channel> ch) {
          ...
          "[outofbox][envelope8]")
{
    ...
    for(int iter = 0; iter < iterations; ++iter)
    {
        ...
        std::vector<std::byte> got;
        server.on_accepted([&](std::unique_ptr<pasio::unix_channel> ch) {
            accepted = std::move(ch);
            accepted->on_data([&](std::span<const std::byte> d) { got.assign(d.begin(), d.end()); });
        });
          ...
          "[outofbox][envelope8]")
{
    ...
    for(int iter = 0; iter < iterations; ++iter)
    {
        ...
        pasio::udp_transport server{io, budget, pasio::udp_transport::arq_type::default_ladder, paced_arq(),
                                    pio::congestion::block, demux::default_max_peers,
                                    k_socket_buf, k_socket_buf, pasio::udp_server::default_send_queue_bytes,
                                    pio::global_default_max_message_bytes, pio::reassembly_memory_budget,
                                    pasio::udp_channel::default_backpressure_bytes, reassembly_timeout};
        pasio::udp_transport client{io, budget,
                                    pasio::udp_transport::arq_type::schedule{ms{20}, ms{40}, ms{80}}, paced_arq(),
                                    pio::congestion::block, demux::default_max_peers,
                                    k_socket_buf, k_socket_buf, pasio::udp_server::default_send_queue_bytes,
                                    pio::global_default_max_message_bytes, pio::reassembly_memory_budget,
                                    pasio::udp_channel::default_backpressure_bytes, reassembly_timeout};
        ...
        REQUIRE(got.size() == 1);
        REQUIRE(equal_bytes(got.front(), payload));   // byte-equal at the shipped ceiling, default caps
        ++proven;
    }
    REQUIRE(proven == iterations);
}
...
struct identity_fixture
{
    ...
    explicit identity_fixture(const std::string &tag)
    {
        dir = std::filesystem::temp_directory_path()
            / ("plexus_oob_" + tag + "_" + std::to_string(reinterpret_cast<std::uintptr_t>(this)));
    ...
    void generate()
    {
        ...
        X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC,
            reinterpret_cast<const unsigned char *>("plexus-oob-tls"), -1, -1, 0);
