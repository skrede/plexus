...
handshake_fsm_config make_cfg(std::uint8_t id_seed, const plexus::io::security::attach_policy *policy)
{
    return handshake_fsm_config{.self_id = make_id(id_seed), .version_major = 1, .version_minor = 0,
...
TEST_CASE("host_identity: a spoofed external claim is ignored — the accessor binds to the authenticated peer",
          "[integration][host_identity]")
{
    admit_policy admit;
    ...
    transport.on_accepted([&](std::unique_ptr<inproc_channel<>> ch) {
    ...
    transport.on_dialed([&](std::unique_ptr<inproc_channel<>> ch, const plexus::io::endpoint &) {
        req_ctx.channel = std::move(ch);
        req_ctx.node_name = "responder-node";
        req_ctx.peer_id = id_resp;
        requester.emplace(req_ctx, ex, make_cfg(0x02, &admit), k_long_timeout,
