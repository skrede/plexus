          ...
          "[integration][observer][tap]")
{
    ...
    net.provider.procedures().serve(rpc_net::k_proc,
                                    [](std::span<const std::byte>, plexus::io::procedure_forwarder<manual_policy>::reply_fn &reply) { reply(plexus::wire::rpc_status::success, {}); });

    rpc_status got = rpc_status::error;
    net.caller.call(rpc_net::provider_id(), rpc_net::k_proc, as_bytes(std::string{"ping"}), [&](rpc_status s, std::span<const std::byte>) { got = s; });
