...
struct link
{
    ...
    {
        transport.on_accepted([this, resp_fp](std::unique_ptr<inproc_channel<>> ch) {
        ...
        transport.on_dialed([this, req_fp](std::unique_ptr<inproc_channel<>> ch, const plexus::io::endpoint &) {
...
TEST_CASE("the same-host verdict gates shared-memory eligibility: a non-same-host pair never attempts a ring acquire",
          "[integration][peer_session][same_host][inproc]")
{
