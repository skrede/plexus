...
struct dial_unix_link
{
    ...
    {
        transport.on_accepted([this, timeout](std::unique_ptr<pasio::unix_channel> ch) {
        ...
        transport.on_dialed([this, timeout](std::unique_ptr<pasio::unix_channel> ch, const pio::endpoint &) {
...
TEST_CASE("unix transport: a DIALED peer_session pair completes the handshake over real AF_UNIX and mints epochs, looped",
...
TEST_CASE("unix transport: a real published message carrying the minted epoch flows post-dial over AF_UNIX, looped",
          "[integration][transport][unix]")
{
    constexpr int k_iterations = 100;
