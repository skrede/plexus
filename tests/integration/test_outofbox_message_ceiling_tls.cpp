...
struct identity_fixture
{
    ...
    {
        dir = std::filesystem::temp_directory_path() / ("plexus_oob_" + tag + "_" + std::to_string(reinterpret_cast<std::uintptr_t>(this)));
    ...
    void generate()
    {
        ...
        X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC, reinterpret_cast<const unsigned char *>("plexus-oob-tls"), -1, -1, 0);
...
TEST_CASE("outofbox: an 8 MiB message round-trips over mutual-TLS at shipped defaults, looped", "[outofbox][envelope8]")
{
