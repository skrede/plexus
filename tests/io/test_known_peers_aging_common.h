...
struct aging_pair
{
    ...
    aging_pair()
            : a(a_transport, ex, make_cfg(0xA1), k_long_timeout, forever_cfg(), k_seed, sink, false, plexus::io::global_default_max_message_bytes, live_opts())
            , b(b_transport, ex, make_cfg(0xB2), k_long_timeout, forever_cfg(), k_seed + 1, sink, false, plexus::io::global_default_max_message_bytes, live_opts())
    {
        b.listen({"inproc", "svc"});
    }
