...
{
    recorder rec;
    send_queue q{rec.sink(), 2};             // finite capacity of two

    REQUIRE(q.enqueue(bytes_of({1}), 1));    // admitted, now in flight
    REQUIRE(q.enqueue(bytes_of({2}), 2));    // admitted, parked behind the front
    REQUIRE(q.full());
