#ifndef HPP_GUARD_PLEXUS_ASIO_MUX_CHANNEL_H
#define HPP_GUARD_PLEXUS_ASIO_MUX_CHANNEL_H
...
// the concrete types are assembled behind the base in this plexus-asio-visible
// header, never enumerated.
//
...
// state no-alloc gate). mux_channel holds one unique_ptr<concrete_channel_base> and
// forwards each verb through ONE virtual hop; the unique_ptr is minted ONCE at dial,
...
public:
    ...
    [[nodiscard]] virtual io::endpoint remote_endpoint() const = 0;
    ...
    virtual void on_error(plexus::detail::move_only_function<void(io::io_error)> cb) = 0;
...
public:
    ...
    [[nodiscard]] io::endpoint remote_endpoint() const override { return m_c->remote_endpoint(); }
    ...
    void on_error(plexus::detail::move_only_function<void(io::io_error)> cb) override { m_c->on_error(std::move(cb)); }
...
{
public:
    explicit mux_channel(std::unique_ptr<concrete_channel_base> impl) : m_impl(std::move(impl)) {}

    mux_channel(const mux_channel &) = delete;
    mux_channel &operator=(const mux_channel &) = delete;
    mux_channel(mux_channel &&) = default;
    mux_channel &operator=(mux_channel &&) = default;

    void send(std::span<const std::byte> data) { m_impl->send(data); }
    void close() { m_impl->close(); }
    [[nodiscard]] io::endpoint remote_endpoint() const { return m_impl->remote_endpoint(); }
    ...
    void on_error(plexus::detail::move_only_function<void(io::io_error)> cb) { m_impl->on_error(std::move(cb)); }
...
static_assert(plexus::io::byte_channel<plexus::asio::mux_channel>,
    "mux_channel must satisfy byte_channel — check the seven erased verbs");
