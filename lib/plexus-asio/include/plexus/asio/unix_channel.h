...
// AF_UNIX byte_channel over ::asio::local::stream_protocol::socket. A near-verbatim
// protocol-type swap of asio_channel: inbound bytes feed a member
...
// unix_channel(io) mints a real default-config channel, never an unarmed one.
class unix_channel
{
public:
    ...
    // the OOM the unbounded queue left possible. Sized at 1x the 4 MiB max-message ceiling
    // so this per-connection FIFO is the SHALLOW socket-facing buffer holding roughly one
    // frame in flight: the deep, priority-ordered backlog lives in the forwarder's egress
    // bands ABOVE this queue, so a deep channel FIFO would silently defeat banding by
    // re-accumulating an un-prioritized backlog. A load-bearing knob — to be substantiated
    ...
    // required-WITH-default ctor args exactly as asio_channel.
    ...
    ~unix_channel() { m_inbound.shutdown(); shutdown_socket(); }

    unix_channel(const unix_channel &) = delete;
    unix_channel &operator=(const unix_channel &) = delete;
    unix_channel(unix_channel &&) = delete;
    unix_channel &operator=(unix_channel &&) = delete;
    ...
    // the would_block stall edge surfaced (never an unbounded deque) — verdicts byte-identical
    // to asio_channel. Compare-before-add so the running total never wraps.
...
private:
    ...
    void shutdown_socket()
    {
        if(!m_socket.is_open())
            return;
        std::error_code ec;
        (void)m_socket.shutdown(::asio::local::stream_protocol::socket::shutdown_both, ec);
