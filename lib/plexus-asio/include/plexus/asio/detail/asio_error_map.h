...
{
    if(ec == ::asio::error::connection_refused) return io::io_error::connection_refused;
    if(ec == ::asio::error::connection_reset)   return io::io_error::connection_reset;
    if(ec == ::asio::error::broken_pipe)        return io::io_error::broken_pipe;
    if(ec == ::asio::error::eof)                return io::io_error::broken_pipe;
    if(ec == ::asio::error::timed_out)          return io::io_error::timed_out;
    if(ec == ::asio::error::address_in_use)     return io::io_error::address_in_use;
    if(ec == ::asio::error::operation_aborted)  return io::io_error::operation_aborted;
