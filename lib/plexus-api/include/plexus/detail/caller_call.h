...
void dispatch_bytes_call(const io::endpoint_seam &seam, std::string_view fqn,
                         std::span<const std::byte> param, Completion &&completion,
...
expected<Res, std::error_code> resolve_typed_reply(std::optional<wire::rpc_status> status,
                                                   std::span<const std::byte>      bytes,
                                                   ResponseCodec                  &res_codec)
{
...
void dispatch_typed_call(const io::endpoint_seam &seam, std::string_view fqn,
                         std::span<const std::byte> encoded, ResponseCodec res_codec,
                         Completion &&completion, std::optional<std::chrono::nanoseconds> deadline)
{
    seam.call(
            seam.ctx, fqn, encoded,
            [completion = std::forward<Completion>(completion), res_codec = std::move(res_codec)](
                    std::optional<wire::rpc_status> status, std::span<const std::byte> bytes,
