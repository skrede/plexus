...
// A move-only handle owning a pool slot until it is either published (the slot pointer is nulled,
// passing the release obligation to the in-flight carrier refcount) or destroyed (an un-published
// handle returns its slot to the pool). A published handle is left null so its destructor releases
// nothing, closing the double-release a published-then-destroyed handle would otherwise cause.
template<typename T>
class loan
{
public:
...
// Exhaustion is not an error and never blocks or grows: try_borrow returns an empty loan and the
// publisher degrades to the serialize path.
template<typename T>
class loan_pool
{
public:
    ...
    // The slot's refcount starts at 1 (the loan's own reference); a publish transfers that
    // reference to the in-flight carrier.
    ...
    // The loan is left null, its release obligation passed to the carrier's refcount. The forwarder
    // addrefs per fast-path send and releases once after the fan loop, so the slot stays balanced.
