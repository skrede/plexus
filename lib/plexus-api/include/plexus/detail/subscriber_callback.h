...
// The arity is resolved once here (the cold path) so the hot demux fans a uniform signature with no
// per-frame branch.
...
// The heap-stable decode state the node's adapters reference by raw pointer. slot is reused
// per-handle, so there is no per-message allocation.
...
struct subscriber_state
{
    ...
    // A decode failure drops — never a partial T, never a teardown.
    ...
    // The demux has already native-key-matched, so the carrier's slot holds a T.
...
// One registration carries both a bytes decode adapter and a process-tier object-dispatch entry
