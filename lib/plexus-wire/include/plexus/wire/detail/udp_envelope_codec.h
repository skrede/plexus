...
// Wrap into a caller-owned buffer reused across calls.
...
// Decode an untrusted datagram, fail-closed on a buffer shorter than the fixed overhead. Reserved
// flag bits are decoded-PERMISSIVE (an unknown future bit is silently ignored) — the deliberate
// OPPOSITE of the strict handshake mode-byte check: a wire KIND must be exact, a flag bit must be
// forward-compatible.
...
// Wrap with the FRAGMENTED bit set but WITHOUT the 10-byte sub-header: the reliable-ARQ fragment
// path carries its msg_id/index/count INSIDE the ARQ segment payload and only needs the envelope
// bit to flag the peer.
...
// Wrap one fragment (best_effort path): the 3-byte envelope with the FRAGMENTED bit set, the
// 10-byte fragment sub-header, then the fragment bytes. Reused caller buffer.
...
// Encode the BARE fragment sub-header + slice — [msg_id:2][frag_idx:4][frag_cnt:4][slice] — with
// no envelope (the reliable-ARQ fragment payload, riding as one ARQ segment). Reused caller buffer.
...
// Decode the fragment sub-header from an untrusted inner frame, fail-closed on a frame shorter
// than the 10-byte sub-header. The returned payload is a view into the caller's frame.
