...
// cross-process consumer; the registry's notifier-driven drain pumps received messages into
// on_data as header-on bytes. close() releases the ring (the demand-driven refcount tears it
// down + unlinks at 1 -> 0). on_protocol_close never fires (no byte-stream framing on a ring).
// Borrows the registry + channel BY REFERENCE; the registry outlives every channel it mints.
...
public:
    ...
    // message_too_large; a stalled reliable path surfaces would_block. The channel stays open
    // either way (the bytes never left the node) — the publisher learns the message will not
    // send rather than seeing a silent drop.
    ...
    // member's entry); the erasure forwards it so a mux-composed shm member keys the scheduler
    // band map exactly as a stream member.
    ...
    // notifier wake (the registry's drain is a discard; this delivers).
    void pump()
    {
        if(!m_on_data)
            return;
...
// The RAII receive lane over a co-host companion ring: it holds ONE request-direction ring
// refcount and installs a consumer sink the registry's armed notifier drains framed messages
// into (posted on the node executor — NO drain thread). Destruction clears the sink BEFORE
// dropping the ring refcount (disarm-before-destroy), so a wake in flight never delivers onto
// a torn-down sink and a release at 1->0 unmaps the ring. Single-owner; non-copy/non-move.
template<typename Broker, typename Notifier>
    requires region_broker<Broker> && notifier<Notifier>
