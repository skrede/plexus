...
void register_session_consumers(Session &s)
{
    s.m_router.on_handshake_req(
            [&s](std::span<const std::byte> inner)
            {
                if(auto req = wire::decode_handshake_request(inner))
                {
                    ...
                    s.m_negotiator.assemble(*req, security::attach_role::responder, req->id,
                                            s.m_fsm_cfg.self_id,
                                            s.m_fsm.attach_policy() != nullptr);
                    s.m_negotiator.latch_peer_proof(*req);
                    if(s.m_negotiator.posture_mismatched(*req))
                        return refuse_posture(s);
                    execute(s,
                            s.m_fsm.on_request(*req, s.m_is_inbound_bootstrap,
                                               s.m_negotiator.facts()));
                }
            });
    s.m_router.on_handshake_resp(
            [&s](std::span<const std::byte> inner)
            {
                if(auto resp = wire::decode_handshake_response(inner))
                {
                    ...
                    s.m_negotiator.assemble(*resp, security::attach_role::initiator, resp->id,
                                            s.m_fsm_cfg.self_id,
                                            s.m_fsm.attach_policy() != nullptr);
    ...
    s.m_router.on_subscribe(
            [&s](std::span<const std::byte> inner)
            {
                if(auto req = wire::decode_subscribe_request(inner))
                {
                    ...
                    s.m_messages.attach_for_fanout(s.m_msg_peer, req->fqn, subscriber_type_id,
                                                   sub_qos);
                }
            });
    ...
    s.m_router.on_subscribe_response([&s](std::span<const std::byte> inner)
                                     { on_subscribe_response_received(s, inner); });
    s.m_router.on_unidirectional(
            [&s](const wire::frame_header &hdr, std::span<const std::byte> inner)
            { deliver_session_data(s, hdr, inner); });
    s.m_router.on_rpc_request(
            [&s](std::span<const std::byte> inner)
            { s.m_procedures.deliver_request(s.m_rpc_peer, inner, s.m_session_id); });
    s.m_router.on_rpc_response([&s](std::span<const std::byte> inner)
                               { s.m_procedures.deliver_response(s.m_rpc_peer, inner); });
