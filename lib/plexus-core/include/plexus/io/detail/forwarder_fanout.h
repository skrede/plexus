                       ...
                       const subscriber_qos        &sub_qos)
{
    ...
    if(sub_qos.posture == attach_posture::strict && subscriber_type_id &&
       !f.m_endpoint.registry().producer_type_id(hash))
    {
        auto resp = wire::encode_subscribe_response(
    ...
    {
        auto resp = wire::encode_subscribe_response(
                {.topic_hash = hash, .status = Forwarder::status_of(rxo.verdict)});
        f.send_control(p.channel, wire::msg_type::subscribe_response, resp);
    ...
    if(f.m_endpoint.registry().bump_refcount(p.node_name, fqn) != 1u)
        return false;
    f.m_endpoint.registry().add_subscriber(hash, fqn, p.channel, p.node_name, sub_qos,
                                           subscriber_type_id);
    f.emit_qos_change(rxo.verdict == io::rxo_verdict::degraded ? qos_edge::degraded
                                                               : qos_edge::accepted,
