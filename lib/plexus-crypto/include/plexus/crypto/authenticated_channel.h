...
private:
    ...
    // One deterministic rekey step: re-derive from the retiring key as IKM under a
    // fixed rekey label. Both directions run the identical chain so a receiver tracks
    // the sender's epoch by re-deriving, never by exchanging fresh key material.
