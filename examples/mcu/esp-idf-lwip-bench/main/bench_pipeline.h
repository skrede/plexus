...
// congestion=block, so the throughput axis of the request/reply pattern is how deep a pipeline the link
// + MCU sustain. At K=1 it reduces to the single-in-flight latency probe (one slot, refill-on-reply).
inline constexpr std::size_t k_max_window = 16;
// A slot stalled past a real round trip is re-issued so a lost reply (or a pre-subscription startup miss)
// cannot wedge the window; well above the slowest real round trip (the 4096 B serial leg rationale).
...
// Emit the pipeline throughput line (completed round trips over the sampling window at depth K); the
// per-completion latency samples reuse emit_sample so p50/p99 latency-at-depth aggregates through the
// existing parser. K rides the policy label (e.g. lwip-p2-k4) so both lines carry the depth.
...
struct windowed_probe
{
    ...
    // A slot re-issue on stall: a genuinely lost reply (or the startup race) frees the window without a
    // record, so the depth is restored and the sweep never wedges. Called from the run loop's pace timer.
    void pump_stalled()
    {
        if(!running)
            return;
...
// The stall-insurance pump: a coarse re-arming timer (well above a real round trip, so it never busy-spins
// the drain) that re-issues stalled slots — the pipeline throughput itself is driven by reply->refill.
