...
io::security::cookie_secret make_cookie_secret()
{
    ...
    io::security::rand_fn rand = [](std::span<std::byte> out) { return RAND_bytes(reinterpret_cast<unsigned char *>(out.data()), static_cast<int>(out.size())) == 1; };
    return io::security::cookie_secret(std::move(hmac), std::move(rand));
}

int dtls_cookie_secret_ex_index() noexcept
{
    return alloc_state_index();
}
int dtls_peer_addr_ex_index() noexcept
{
    return alloc_addr_index();
}
int dtls_peer_facts_ex_index() noexcept
{
    return alloc_facts_index();
}
