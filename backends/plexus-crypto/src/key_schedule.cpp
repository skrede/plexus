...
bool hkdf(int mode, std::span<const unsigned char> key, std::span<const unsigned char> salt_or_info, std::span<unsigned char> out)
{
    ...
    params[n++]        = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, const_cast<unsigned char *>(key.data()), key.size());
    const char *region = mode == EVP_KDF_HKDF_MODE_EXTRACT_ONLY ? OSSL_KDF_PARAM_SALT : OSSL_KDF_PARAM_INFO;
    params[n++]        = OSSL_PARAM_construct_octet_string(region, const_cast<unsigned char *>(salt_or_info.data()), salt_or_info.size());
...
bool expand_into(std::span<const unsigned char> master, std::string_view label, std::span<const std::byte, 32> transcript, aead_key &out)
{
    const auto info = info_for(label, transcript);
    return hkdf(EVP_KDF_HKDF_MODE_EXPAND_ONLY, master, info, std::span<unsigned char>{as_uc_mut(out.data()), out.size()});
}
                 ...
                 std::span<const std::byte, 32> transcript_digest, derived_keys &out)
{
    ...
    if(!hkdf(EVP_KDF_HKDF_MODE_EXTRACT_ONLY, std::span<const unsigned char>{as_uc(psk.data()), psk.size()}, std::span<const unsigned char>{salt}, std::span<unsigned char>{master}))
        return false;

    return expand_into(master, "plexus aead snd", transcript_digest, out.k_send) && expand_into(master, "plexus aead rcv", transcript_digest, out.k_recv);
}
