...
// DTLS records ride RAW (DTLS owns RFC 6347 anti-replay + handshake retransmit). listen(ep) arms
// the recv loop (a MISS mints a server channel behind the cookie gate — OpenSSL replies
// HelloVerifyRequest, no full state until a valid cookie echoes). dial(ep) kicks the client
// handshake, firing on_dialed ON external_complete (the crypto handshake IS the plexus handshake).
