#ifndef HPP_GUARD_PLEXUS_ASIO_SHM_MACOS_SEM_NOTIFIER_H
#define HPP_GUARD_PLEXUS_ASIO_SHM_MACOS_SEM_NOTIFIER_H
...
// The cross-process wakeup -> reactor bridge for macOS, where there is no shared-page futex and no
// fd-shaped cross-process primitive: a named POSIX counting semaphore both peers derive from the
// region name carries the wake. ONE lifetime-bounded thread OFF the executor sem_waits on it, then
// pokes a process-local self-pipe an asio read handler drains, POSTS via Policy::post (never a
// serialized wrapper), and re-arms. Non-copy/non-move: the pipe, the semaphore, and the asio
// descriptor hold stable addresses and the in-flight handler captures `this`.
...
public:
    ...
    sem_notifier(typename Policy::executor_type executor, std::atomic<std::uint32_t> &word, std::atomic<std::uint32_t> &park, std::string_view region_name)
            : m_executor(executor)
            , m_word(word)
            , m_park(park)
    ...
    sem_notifier(typename Policy::executor_type executor, std::atomic<std::uint32_t> &word)
            : m_executor(executor)
            , m_word(word)
    ...
    sem_notifier(const sem_notifier &)            = delete;
    sem_notifier &operator=(const sem_notifier &) = delete;
    sem_notifier(sem_notifier &&)                 = delete;
    sem_notifier &operator=(sem_notifier &&)      = delete;
    ...
    void arm(drain_fn drain)
    {
        ...
        ::fcntl(m_pipe[0], F_SETFD, FD_CLOEXEC);
        ::fcntl(m_pipe[1], F_SETFD, FD_CLOEXEC);
        m_doorbell.emplace(executor_io(m_executor), m_pipe[0]);
        watch_doorbell();
    ...
    // close guarantees the worker never pokes a torn pipe; the alive token makes a drain a prior
    // on_doorbell already posted no-op after free.
...
private:
    ...
    void poke_doorbell() noexcept
    {
        ...
        (void)::write(m_pipe[1], &one, sizeof(one));
    }
    ...
    void on_doorbell(const std::error_code &ec) noexcept
    {
        ...
        (void)::read(m_pipe[0], drained, sizeof(drained));
