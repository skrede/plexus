#ifndef HPP_GUARD_PLEXUS_NATIVE_DETAIL_DISCOVERY_CARD_CODEC_H
#define HPP_GUARD_PLEXUS_NATIVE_DETAIL_DISCOVERY_CARD_CODEC_H
...
inline std::vector<discovery::listening_transport> listens_from_card(const std::vector<std::pair<std::string, std::string>> &card)
{
    ...
    for(const auto &[key, value] : card)
    {
        ...
        if(const auto port = discovery::read_transport_port(card, transport))
...
inline void encode_card_announcement(std::vector<std::byte> &scratch, const discovery::service_info &card, std::uint64_t ttl_secs, std::uint8_t flags)
{
    scratch.clear();
    const auto id = discovery::detail::hex_decode(card.metadata.empty() ? std::string_view{} : std::string_view{card.metadata.front().second});
    if(!id)
        return;
